...
class RadioButtonUI1 extends BasicRadioButtonUI {
               ...
               // JTable table = new JTable(this);
                ...
                try {
                        ...
                        Connection con = DriverManager.getConnection(
                                        "jdbc:mysql://127.0.0.1:3306/prempuri_ashram", "root", "lanuk");
                        String query = "select * from donation";
                        Statement st = con.createStatement();
                        ResultSet rs = st.executeQuery(query);

                        while (rs.next()) {
                                String name = rs.getString("donated_for");
                                //String address = rs.getDouble("donation_amount");
                                addRow(new Object[] { name });
                        }
                        for(int i=0;i<100;i++)
                        	addRow(new Object[] { "chirity" });
                } catch (Exception e) {
                }
        }
}